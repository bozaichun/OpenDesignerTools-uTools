...
import {
  parseColor, copyToClipboard, showToast
...
const simResults = computed(() => {
  const rgb = hexToRgb(simColor.value);
  const variants = [];
  for (let i = 0; i < 8; i++) {
...
const textSuggestions = computed(() => {
  const bgHex = textBgColor.value;
  const cand = [
...
function buildContrastResult(ratio, threshold) {
  if (ratio >= threshold) {
...
function runGovCheck() {
  const hex = govColor.value.trim();
  if (!/^#[0-9A-Fa-f]{6}$/.test(hex)) {
    govChecks.value = [{ title: '格式错误', desc: '请输入合法 HEX 如 #1677FF', pass: false }];
    govPassedCount.value = 0;
    return;
  }
  ...
  govChecks.value = checks;
  govPassedCount.value = checks.filter(c => c.pass).length;
}
