...
import {
  getAllSemanticHistorySessions,
  removeSemanticHistorySession,
  removeAllSemanticHistorySessions,
  formatSessionTime
} from './semanticHistoryStorage.js';
import { truncateSessionTitle } from './intelligentColorMatchingUtils.js';
...
const confirmText = computed(() => {
  if (deleteDialogType.value === 'all') {
    return '确定删除全部历史记录吗？删除后不可恢复。';
  }
  if (deleteDialogType.value === 'batch') {
    return `确定删除选中的 ${selectedCount.value} 条历史记录吗？删除后不可恢复。`;
  }
  return `确定删除「${deleteTargetPreview.value}」吗？删除后不可恢复。`;
});

function refreshSessions() {
  sessions.value = getAllSemanticHistorySessions();
}
...
function handlePrimaryDeleteClick() {
  ...
  if (selectedCount.value === 0) {
    showToast(null, '请先选择要删除的记录', 'warning');
    return;
  }
...
function confirmDelete() {
  if (deleteDialogType.value === 'all') {
    ...
    targets.forEach((session) => removeSemanticHistorySession(session));
    refreshSessions();
    exitMultiSelectMode();
    showToast(null, `已删除 ${targets.length} 条记录`, 'success');
  } else if (deleteTarget.value) {
    removeSemanticHistorySession(deleteTarget.value);
    refreshSessions();
  }
  handleCancelDelete();
}
...
onMounted(() => {
  refreshSessions();
  window.addEventListener('semantic-history-changed', refreshSessions);
});

onUnmounted(() => {
  window.removeEventListener('semantic-history-changed', refreshSessions);
});
</script>
...
<template>
  ...
  >
    <div v-if="!sessions.length" class="history-empty">
      ...
      <p>暂无历史记录</p>
      <span class="history-empty-tip">完成一次 AI 配色后将自动保存</span>
    </div>
